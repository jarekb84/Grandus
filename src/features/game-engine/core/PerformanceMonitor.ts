...
export class PerformanceMonitor {
  ...
  constructor(scene: Phaser.Scene) {
    this.fpsText = scene.add.text(10, 10, 'FPS: 0 | Min: 0 | Max: 0 | Entities: 0', { 
      color: '#00ff00', 
      fontSize: '14px' 
    });
  ...
  update(time: number, entityCount: number): void {
    ...
    if (Date.now() - this.startTime > this.resetInterval) {
      console.log(`Performance reset - Min FPS: ${this.minFps}, Max FPS: ${this.maxFps}, Entities: ${this.entityCount}`);
